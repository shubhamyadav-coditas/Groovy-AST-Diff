// ADDED: Enhanced script metadata
String script_name = "storedToDisplayLOV_Enhanced"  // MODIFIED: Enhanced name
String script_version = "2.0"  // ADDED: Version info

Emery.log.error "Start {} script version {}", script_name, script_version  // MODIFIED: Enhanced logging

// ADDED: Configuration validation
String lovName = "test_lov_enhanced"  // MODIFIED: Enhanced LOV name
...
row1901.api = "getLovDisplayValue Enhanced"  // MODIFIED: Enhanced
try{
	...
	error_count++  // ADDED: Error tracking
	}
F.gr5.rows << row1901
...
try{
    String out1 = Emery.lov.getLovDisplayValue("invalid_lov_name_enhanced", locale_id, "7")  // MODIFIED: Enhanced
	row1902.results = out1
	}
catch(Exception e){
	row1902.results = e.getMessage()
    //results = "LOV with name invalid_lov_name_enhanced is not present"  // MODIFIED: Enhanced
	error_count++  // ADDED: Error tracking
	}
F.gr5.rows << row1902

// MOVED: Empty string test moved before null test
//getLovDisplayValue_empty_string_as_SV
def row1903 = F.gr5.newRow()
row1903.instance_rec_num = 1903
row1903.api = "getLovDisplayValue_empty_string_as_SV Enhanced"  // MODIFIED: Enhanced
try{
...
row1906.api = "getLovDisplayValue_invalid_SV Enhanced"  // MODIFIED: Enhanced
try{
    String out1 = Emery.lov.getLovDisplayValue(lovName, locale_id, "210i4_enhanced")  // MODIFIED: Enhanced
	if(out1 == null){
...
// ADDED: Intermediate performance check
performanceTracker.phase1 = System.currentTimeMillis() - startTime

//getLovDisplayValues
def row1907 = F.gr5.newRow()
row1907.instance_rec_num = 1907
row1907.api = "getLovDisplayValues Enhanced"  // MODIFIED: Enhanced
try{
    Map<String, String> out1 = Emery.lov.getLovDisplayValues(lovName, locale_id, ["5", "7", "9"])  // MODIFIED: Added "9"
	row1907.results = out1
	//results = {5=five, 7=seven!@#`,:;'(=>)"%$#@%^&, 9=nine}  // MODIFIED: Updated result
	}
catch(Exception e){
...
// MOVED: Multi-locale tests moved up
//getMultiLocaleLovDisplayValue
def row1912 = F.gr5.newRow()
row1912.instance_rec_num = 1912
row1912.api = "getMultiLocaleLovDisplayValue Enhanced"  // MODIFIED: Enhanced
try{
    Map<Integer, String> out1 = Emery.lov.getMultiLocaleLovDisplayValue(lovName, [1009, 1008], "7")  // MODIFIED: Added 1008
	row1912.results = out1
	//results = {1009=seven!@#`,:;'(=>)"%$#@%^&, 1008=seven_locale_1008}  // MODIFIED: Updated result
	}
catch(Exception e){
...
//********** Get Store Values Enhanced ******  // MODIFIED: Enhanced comment

//getLovStoredValue
def row1916 = F.gr5.newRow()
row1916.instance_rec_num = 1916
row1916.api = "getLovStoredValue Enhanced"  // MODIFIED: Enhanced
try
{
	String singleStoredValue_valid = Emery.lov.getLovStoredValue(lovName,locale_id, "five_enhanced")  // MODIFIED: Enhanced
...
row1918.api = "getLovStoredValues Special Char Enhanced"  // MODIFIED: Enhanced
try
{
	String specialCharStoredValue_valid = Emery.lov.getLovStoredValue(lovName,locale_id, "seven!@#,:;=*%\$#@%^&_enhanced")  // MODIFIED: Enhanced
...
row1919.api = "getLovStoredValues Invalid Enhanced"  // MODIFIED: Enhanced
try
{
	String invalid_name = Emery.lov.getLovStoredValue("ABCDEF_ENHANCED",locale_id, "Turn OFF logging enhanced")  // MODIFIED: Enhanced
	row1919.results = invalid_name  // MODIFIED: Actually store the result
	}
catch (Exception e){
	Emery.log.error("TESTLOG: Exception while lov name LOV NAME : ABCDEF_ENHANCED" + e.getMessage() +"  "+ e.getClass())  // MODIFIED: Enhanced
	row1919.results = e.getMessage()
	error_count++  // ADDED: Error tracking
	}
F.gr5.rows << row1919

// ADDED: Error summary and performance report
def row1920 = F.gr5.newRow()
row1920.instance_rec_num = 1920
row1920.api = "Script Summary Report"
try {
    def totalTime = System.currentTimeMillis() - startTime
    performanceTracker.total = totalTime
    row1920.results = "Errors: ${error_count}, Total Time: ${totalTime}ms, Phase1: ${performanceTracker.phase1}ms"
}
catch(Exception e){
    row1920.results = e.getMessage()
}
F.gr5.rows << row1920

// DELETED: The original end message is removed

Emery.log.error "End {} script version {} - Enhanced execution completed with ${error_count} errors", script_name, script_version  // MODIFIED: Enhanced end message