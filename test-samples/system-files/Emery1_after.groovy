// ADDED: Enhanced script header with version info
Emery.log.error "Start script - Version 2.0 Enhanced"

// ADDED: New initialization block
def scriptStartTime = new Date()
Emery.log.info "Script execution started at: ${scriptStartTime}"

def row1 = F.gr5.newRow()
row1.instance_rec_num = 1
row1.api = "formId"
...
row2.api = "Workflow Code"
row2.results = Emery.form.workFlowCode("MS_ATD_EMERYAUTOMATION2")
F.gr5.rows << row2

def row3 = F.gr5.newRow()
row3.instance_rec_num = 3
row3.api = "Blueprint Code Enhanced"  // MODIFIED: Enhanced description
...
row4.results = name["FIRST_NAME"] + " " + name["LAST_NAME"]
F.gr5.rows << row4

def row5 = F.gr5.newRow()	
row5.instance_rec_num = 5
row5.api = "User ID for user pfadmin"
...
// MOVED: Config parameter moved up and uncommented
def row7 = F.gr5.newRow()	
row7.instance_rec_num = 7
row7.api = "Config Parameter Lov Enhanced"  // MODIFIED: Enhanced and uncommented
...
//F.description = "Running automation for EMERY API's using Autometric Enhanced" // MODIFIED: Enhanced description
def row11 = F.gr5.newRow()	
row11.instance_rec_num = 11  // MODIFIED: Fixed duplicate instance_rec_num
row11.api = "Description"
row11.results = "Running automation for EMERY API's using Autometric Enhanced"  // MODIFIED: Enhanced
F.gr5.rows << row11
...
try{
	row12.results = Emery.util.getUserId("")
	}
catch(Exception e){
...
// ADDED: New validation row
def row13a = F.gr5.newRow()
row13a.instance_rec_num = 13
row13a.api = "Additional User Validation"
try{
	row13a.results = Emery.util.getUserId("testuser")
}
catch(Exception e){
	row13a.results = "Test user validation: " + e.getMessage()
}
F.gr5.rows << row13a
...
newRow.updateField("api","Current Stage Enhanced");  // MODIFIED: Enhanced
...
newRow4.updateField("api","Status Name of the GRC DataTable Enhanced");  // MODIFIED: Enhanced
newRow4.updateField("results",targetRecord1.STATUS_NAME);
newRow4.updateField("unique_row_id",Emery.util.nextUniqueRowId())
gr5.addRow(newRow4);

//String Utils
String encode1 = "M\"etricstream\", infotech, pvt, ltd, enhanced"  // MODIFIED: Added enhanced
...
String decode = "M\"\"etricstream\"\", infotech, pvt, ltd, enhanced"  // MODIFIED: Added enhanced
...
/********Locales Brazos 2021 Enhanced *******/  // MODIFIED: Enhanced comment

BiMap<Long, String> locales = Emery.util.fetchLocales(LocaleType.ALL)
String locale =  locales.get(Long.valueOf(1001))
Emery.log.error "All Locales Enhanced {}",locale  // MODIFIED: Enhanced logging
...
// ADDED: New performance tracking
def scriptEndTime = new Date()
AbstractRow newRow9 = (AbstractRow) gr5.newRow(); 
newRow9.updateField("api","Script Execution Time");
newRow9.updateField("results","Duration: " + (scriptEndTime.time - scriptStartTime.time) + "ms");
newRow9.updateField("unique_row_id",Emery.util.nextUniqueRowId())
gr5.addRow(newRow9);

F.save();

Emery.log.error "Autometric Framework Enhanced - Execution Completed"  // MODIFIED: Enhanced final message